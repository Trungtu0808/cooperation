...
extension DataExtendNumNotNull on num {
  int toPage(int limit) {
    return this ~/limit + 1;
  }
}
...
extension DataExtendNum on num? {
  ...
  String toSecondsConvertMinute(){
    int minutes = (this! / 60).round();
    String minutesStr = (minutes).toString().padLeft(2, );
    return minutesStr;
  }

  String toSecondsConvertHour(){
    int hour = (this! / 3600).round();
    String minutesStr = hour.toString().padLeft(2, );
    return minutesStr;
  }
  ...
  String toCountThousandStr() {
    return this.isNullOrEmpty() ? '0' : (this! > 999 ? '1k+' : this.toString());
  }

  String toThousandSeparatorWithZero() {
    return PriceExtend.strToNumberFormat(this.toString()) ?? '0';
  }
  ...
  String toThousandSeparatorVND() {
    return '${PriceExtend.strToNumberFormat(this.toString())}đ';
  }
...
extension DataExtendString on String? {
  ...
  String? get getNumberString{
    var text = this;
    if ((int.tryParse(text ?? "") ?? 0) < 10){
      return "0$text";
    }
    return text;
  }
